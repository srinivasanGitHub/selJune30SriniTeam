...
public class ValidPassword {
	...
	public static void main(String[] args) {
		...
		passWord = objScr.nextLine();
		objScr.close();
		
		passPattern="(((?=.*d){2})((?=.*[a-z]){2})(?=.*[A-Z])(?!.*[^a-zA-Z0-9]).{10,20})";
		...
		else
		{
			...
			System.out.println("password must contains Atleast two letters,two digits,one uppercase letter "
					+ "and minlength should be ten and aslo with no special symbols");
		}
	}
